...
class model(object):
    def __init__(self, data, num_class, parameters):
        ...
        with tf.name_scope('lstm1'): 
            with tf.variable_scope('lstm1'):
                ...
                for time_step in range(self.n_steps):
                    ...
                    (cell_h, state, dw) = lstm_cell(self.lstm_input[:, time_step, :], state, time_step)
                    cell_output = cell_h
                    self.lstm_outputs.append(cell_output) 
                    self.dweight.append(dw) 
                self.dynamic_weight = tf.stack(self.dweight, axis = 1)
                self.lstm_output_final = tf.reshape(self.lstm_outputs[-1], [-1, self.lstm_hidden_size])
        ...
        with tf.name_scope('accuracy'):
            self.prediction_label = tf.argmax(self.prediction, 1)
            self.true_label = tf.argmax(self.y_one_hot, 1)
            correct_predict = tf.equal(self.prediction_label, self.true_label)
