...
from keras.layers import Dense, BatchNormalization, Activation, Convolution2D, MaxPooling2D, Flatten, Input, concatenate, Lambda
...
class TPRNNBasicModel(object):
  ...
  def tprnn_one_step(self, current_inputs, current_states, lstms, step_counter, previous_contexts):
    ...
    for i in range(len(current_inputs)):
      with tf.variable_scope("lstm_" + (str(0) if i == 0 else str(1)) ):
        ...
        if i == 0: # first level
          output_state_pair = lstms[0](current_inputs[i], current_states[i], step_counter)
        elif i == 1: # second level, first phase
          if step_counter % 2 == 1:
            output_state_pair = lstms[1](current_outputs[0], current_states[i], step_counter)
          else:
  ...
  def setup_context_lstm(self):
    ...
    with tf.variable_scope("setup_context_lstm"):
      ...
      lstms = [rnn_cell_impl.TAMSRNN_CELL(self.rnn_size, self.periods, state_is_tuple=False) for i in range(2)]
